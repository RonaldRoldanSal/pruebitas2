...
function lista_usuarios(valor,pagina){
    // valor = valor que el usuario escribe para mostrar los datos por email o nombre
    var pagina=Number(pagina);
    $.ajax({
        url:'../Controller/usuarios/usuarios_controller_listar.php',
        type:'POST',
        data:'valor='+valor+'&pagina='+pagina+'&boton=buscar',
        beforeSend: function(){
            $("#loading_usuario").addClass("fa fa-refresh fa-spin fa-3x fa-fw");
        },
		...
		var valores = eval(datos[0]); //me trae solo los datos menos el numero de filas
                ...
                for (var i=0; i<valores.length;i++){
                    ...
                    cadena += "<td><div class='btn-group'> <button type='button' class='btn btn-info ' data-toggle='dropdown' aria-expanded='false'>Acción <span class='glyphicon glyphicon-cog'></span></button> <button type='button' class='btn btn-default dropdown-toggle' data-toggle='dropdown' aria-expanded='false'> <span class='caret'></span></button> <ul class='dropdown-menu' role='menu'> <li><a href='#' data-toggle='modal' data-target='#myModal_modificar' onclick='mostrar_usuarios("+'"'+datos_array+'"'+");'>Modificar</a></li> <li class='divider'></li> <li><a href='#' data-toggle='modal' data-target='#myModal_eliminar'  onclick='eliminar_usuarios("+'"'+datos_array+'"'+");' >Eliminar</a></li> </ul> </div></td>";
                    cadena += "</tr>";
                }
                ...
                var buscar_usuario = $("#buscarusuarios").val();
                
                var paginar = "<ul class='pagination'>";
                if (pagina>1){
                    paginar += "<li><a href='javascript:void(0)' onclick='listar_usuarios("+'"'+buscar_usuario+'","'+1+'"'+")'>&laquo;</a></li>";
                    paginar += "<li><a href='javascript:void(0)' onclick='listar_usuarios("+'"'+buscar_usuario+'","'+(pagina-1)+'"'+")'>Anterior</a></li>";   
                }
                else {
                ...
                for (i=pagina_inicio;i<=pagina_fin;i++){
                    if(i==pagina){
                        ...
                        paginar += "<li><a href='javascript:void(0)' onclick='listar_usuarios("+'"'+buscar_usuario+'","'+i+'"'+")'>"+i+"</a></li>";
                    }
                }
                
                if (pagina<numero_paginas){
                    paginar += "<li><a href='javascript:void(0)' onclick='listar_usuarios("+'"'+buscar_usuario+'","'+(pagina+1)+'"'+")'>Siguiente</a></li>";
                    paginar += "<li><a href='javascript:void(0)' onclick='listar_usuarios("+'"'+buscar_usuario+'","'+numero_paginas+'"'+")'>&raquo;</a></li>";
                }
                else {
